...
const sendMMS = async ({ from, to, message, attachments }) => {
  logger.info("sending MMS with " + attachments.join(' '));
...
const makeTag = (from, to) => {
  const tag = ethers.BigNumber.from(ethers.utils.solidityKeccak256(["string", "string"], [to, from])).mod(ethers.BigNumber.from(10).pow(4)).toString();
  return padTag(tag);
};
    ...
    if (await fallbackForDID(sms.to) === sms.from) {
      ...
      if (matches) {
        ...
        if (target) {
          ...
          logger.info('RELAY OUT (' + (await redis.get('extfor.' + sms.to)) + ':' + tag + ')');
      ...
      if (matches) {
        logger.info('RELAY OUT (' + (await redis.get('extfor.' + sms.to)) + ':' + matches[1] + ')');
        return await redis.lpush(SMS_OUT_CHANNEL, JSON.stringify(Object.assign({}, sms, {
          from: sms.to,
          to: matches[1],
          message: matches[2]
...
const getOffset = () => {
  return (isDST() ? 4 : 5)*3600;  // Eastern Time: EDT=UTC-4, EST=UTC-5
};
const fromUnix = (unix) => {
...
const toArray = (s) => {
  const clone = { ...s };
  const length = Math.max(...Object.keys(s).filter((v: any) => !isNaN(v)).map(Number))
  clone.length = length;
  ...
  if (sms)
    await sms.reduce(async function pull(r, v, i, ary, count = 0) {
      ...
      if (msg.attachments.length === 0 && msg.message === '') {
        logger.info('Must pull again:');
        logger.info(util.inspect(msg, { colors: true, depth: 5 }));
        await new Promise((resolve) => setTimeout(resolve, 1500));
        return await pull(r, ((await voipms.getMMS.get({ id: v.id })).sms || {})[0] || {}, i, ary, count + 1);
      }
      if (count === 5) return;
      const result = await handleSms(msg).catch((err) => logger.error(err));
...
const forwardMessage = async (msg) => {
  ...
  logger.info(util.inspect(decoded, { colors: true, depth: 2 }));
  if ((decoded.message || "").length > 160 || decoded.attachments.length)
    await sendMMS(decoded);
  else await sendSMPP(decoded);
  return true;
...
const startStatusServer = () => {
  ...
  server.listen(8080, '0.0.0.0', () => {
    logger.info('Status server listening on port 8080');
  });
  return server;
};
